...
- **Actions sur un terme depuis l'éditeur** : le menu contextuel de `GlossaryForm` ferme l'éditeur avec un `GlossaryTermAction` (terme, langue, Contrôler ou Retraduire) ; `BtnGlossary_Click` l'exécute par `RunGlossaryTermActionAsync` **avant** la détection automatique, qui écarte ensuite ce terme dans cette langue (`GlossaryImpact.RemoveTerm`) pour ne pas redemander les mêmes lignes. Les lignes visées sont celles dont le français contient le terme (`GlossaryImpact.SelectImpactedRows`, même inclusion que la retraduction ciblée — « disjoncteurs » compte pour « disjoncteur ») ; la confirmation rappelle la cellule imposée et le compte d'écarts parmi elles (`GlossaryDeviation.SelectDeviationsForTerm` sur `ControlledEntries`), ou dit pourquoi aucune contrainte ne s'appliquera (cellule vide, terme non Validé donc absent de `GetPromptEntries`). **Contrôler** (`VerifyTermRowsAsync`) bascule la grille sur la langue, vérifie les lignes traduites par `VerifyRowsAsync` (vue active, glossaire avec garde-fou), pousse la vue active dans les dictionnaires, marque les lignes et filtre sur `translation:review`. **Retraduire** (`RetranslateTermRowsAsync`) propose dans un `TaskDialog` les écarts seuls (par défaut, quand ils sont strictement moins nombreux que les lignes) ou toutes les lignes, puis enchaîne sur `RetranslateImpactedAsync` pour cette seule langue.
...
- **Une action demandée depuis l'éditeur écarte sa paire (terme, langue) de la détection automatique** — `ProposeTargetedRetranslationAsync(projectionBefore, alreadyRequested)` passe par `GlossaryImpact.RemoveTerm`. Sans cela, corriger la cellule allemande de « disjoncteur » puis cliquer « Retraduire … en Allemand » produirait deux confirmations pour les mêmes lignes. Les autres changements faits dans l'éditeur restent proposés normalement.
