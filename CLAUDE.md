...
- **F5 se bloque dans `CanRefreshView`, pas dans la toolbar** — le raccourci passe par `ProcessCmdKey`, au niveau de la fenêtre : désactiver `toolStrip` ne l'arrête pas. `CanRefreshView` vérifie `toolStrip.Enabled` et couvre ainsi les trois gels (batch IA, écriture disque, analyse de mise en page). Ne pas retirer cette condition : le bouton Rafraîchir grisé ne protégerait plus rien au clavier.
