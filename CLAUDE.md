...
4. [Format .resx (lecture directe)](#4-format-resx-lecture-directe)
...
CheckTranslation est une application de bureau **Windows Forms** en **C# / .NET 8** (`net8.0-windows`) destinée à **contrôler, traduire et vérifier** des chaînes de ressources `.resx`. Elle travaille **directement sur les fichiers `.resx`** d'une solution (`.sln` / `.slnx`) : les projets sont parcourus, chaque `.resx` neutre fournit le français, ses variantes de culture les traductions.

L'application affiche les entrées dans un `DataGridView`, permet l'édition de la traduction dans une langue cible, puis réécrit les seules variantes de culture modifiées. (La lecture d'un export Excel ResX Resource Manager et la fusion Excel ont existé jusqu'en septembre 2026 ; elles ont été retirées, la solution étant la seule source.)

Elle peut appeler les API **OpenAI** et **Anthropic**, en direct ou au travers de la passerelle **Bifrost**, pour traduire et vérifier des traductions en lot, avec cache mémoire et glossaire métier injecté dans les prompts.
...
### Dépendances NuGet
- `ClosedXML` `0.104.2` — export / import Excel du glossaire (contrôle externe)
...
│   ├── GlossaryTermAction.cs            # Demande portée par l'éditeur de glossaire à sa fermeture (contrôler / retraduire un terme dans une langue)
│   └── LayoutStatus.cs                  # Enum verdict de mise en page (NotChecked / Ok / Truncated / Collision / Unverifiable)
├── Services/
│   ├── ITranslationSource.cs                 # Abstraction de source (Load / Save)
│   ├── ITranslationSourceFactory.cs / TranslationSourceFactory.cs  # .sln/.slnx → resx
...
│   ├── SolutionGlossaryLocator.cs            # .claude/glossary.json à côté de la solution (glossaire partagé avec les skills)
...
│   ├── GlossaryCandidates.cs                 # Langues avec contenu + fusion des candidats d'extraction multi-langues (pure)
│   └── GlossaryDeviation.cs                  # Écarts au glossaire, port de glossary.py check (pure)
├── Controls/
...
Ces conventions reproduisent les colonnes `Project` / `File` / `Key` de l'export ResX Resource Manager : l'identité `Project\|File\|Key` reste compatible avec l'outillage qui raisonne en ces termes (dont `resx-tools` d'elec calc).
...
- `// --- Arborescence de la solution (style ResX Resource Manager) ---` : panneau gauche (`SplitContainer` créé par code, grille du Designer réparentée dans `Panel2`), arbre Projet → Fichier à cases à cocher. Décocher restreint l'affichage de la grille (`_uncheckedFiles`, identité projet + fichier), bouton toolbar pour replier le panneau, largeur et visibilité persistées. Les projets se replient (boutons de repli des racines : `ShowLines` + `ShowRootLines`, thème Explorer pour des flèches sans pointillés), l'état des replis vit dans `_collapsedProjects`, hors des nœuds, et survit aux reconstructions.
...
**`Forms/GlossaryForm.cs`** — éditeur du glossaire transversal : une ligne par terme, une colonne par langue (créées par code depuis `MainForm.Languages`), plus Statut (Proposé / En contrôle / Validé) et Commentaire réviseur (lecture seule, rempli par l'import). Grille non liée : les dictionnaires de `GlossaryTerm` ne se prêtent pas au binding. Doublons de Source refusés à l'enregistrement, terme saisi à la main Validé par défaut, confirmation si modifications non enregistrées. **Non modal** (`Show(owner)`, une seule instance tenue par `MainForm._glossaryForm`, le bouton ramène l'éditeur ouvert au premier plan) : la grille principale reste utilisable pendant l'édition, et tout ce qui suivait `ShowDialog` — action demandée sur un terme, détection des lignes impactées — vit dans `MainForm.GlossaryForm_FormClosed`. Conséquence WinForms : un bouton à `DialogResult` ne ferme pas une fenêtre non modale, `btnCancel.Click` appelle `Close()` explicitement. **Menu contextuel d'un terme** (clic droit sur une ligne ; `grid.EndEdit()` d'abord, une source tapée dans une ligne neuve n'est dans `Value` qu'après — et comme ce commit peut re-trier la grille, colonnes en `SortMode.Automatic`, la ligne est tenue par référence, jamais par index, et le menu s'ouvre sous la souris) : « Filtrer la grille principale sur « terme » » (la grille ne montre plus que les lignes dont le français contient le terme, sans fermer l'éditeur — délégué `FilterMainGrid` posé par `MainForm`, qui rend le compte affiché dans le bandeau de l'éditeur, ou -1 grille gelée), puis « Contrôler les traductions de « terme » en <langue> » et « Retraduire les traductions de « terme » en <langue> » — pour ces deux-là la langue est celle de la colonne cliquée, ou, hors des colonnes de langue, celle que la grille principale affiche au moment du clic (délégué `MainGridLanguageCode` posé par `MainForm`, lu à chaque clic : l'éditeur étant non modal, la grille a pu changer de langue depuis son ouverture), et l'entrée la nomme toujours. L'éditeur n'a pas accès aux lignes : il enregistre d'abord les modifications en cours si l'utilisateur l'accepte (l'action porte sur le glossaire enregistré, celui que les prompts verront), puis se ferme en portant la demande dans `RequestedAction` (`GlossaryTermAction`), que `MainForm` exécute — même mécanique que le drill-down du tableau de bord. `TrySaveTerms` est le chemin d'enregistrement commun au bouton Enregistrer et à ces actions.

**`Forms/GlossaryExtractionDialog.cs`** — validation des candidats d'extraction : une ligne par terme, une colonne par langue extraite (une seule en mode « langue courante », toutes celles avec du contenu en mode multi-langues), colonnes créées par code à `SetCandidates(candidats, langues, termesExistants)` comme dans `GlossaryForm`. Grille non liée (les dictionnaires de `GlossaryTerm` ne se prêtent pas au binding). Chaque cellule est confrontée au glossaire courant par `GlossaryCandidates.Classify` — la même définition que le versement — et colorée : **vert** sera écrit (terme nouveau, ou case vide remplie), **noir** est déjà au glossaire (affiché tel que le glossaire le porte, en lecture seule, pour que personne ne ressaisisse une valeur qui serait abandonnée), **rouge** diffère d'une valeur déjà tranchée et ne sera pas appliqué (valeur tranchée en infobulle). Une ligne qui n'ajouterait rien arrive décochée ; l'en-tête compte nouveaux / à compléter / sans rien à ajouter et porte la légende — sa hauteur suit le texte replié dans la largeur courante (`FitHeaderHeight`, à l'affectation et au `Resize` du panneau), sans quoi le panneau fixe rognerait à largeur minimale. L'utilisateur coche, corrige éventuellement, puis valide ; `AcceptedTerms` ne rend que les termes cochés qui ont une source et au moins une cellule non vide — cellules noires et rouges comprises, que le service reclassera et n'écrira pas.

### 6.3 Services

**`ITranslationSource`** — abstraction de la source. Une instance est liée à un chemin et expose `Path`, `Kind` (« resx », affiché dans la status bar), `SupportsLayoutCheck`, `Load(languages, progress)` et `Save(rows, languages)`. Une seule implémentation aujourd'hui, `ResxTranslationSource` ; l'abstraction et `TranslationSourceFactory` (qui expose aussi `OpenFileFilter`, limité aux `.sln` / `.slnx`) survivent au retrait de la source Excel : elles isolent l'interface de la lecture disque et restent le point d'accroche d'une source future.
...
- **Elles n'existent que dans les `.resx`** des formulaires : un dictionnaire de messages n'a pas de géométrie, ses lignes restent `NotChecked`.
...
- Stockage **transversal** (`GlossaryTerm` : un terme français, ses traductions par langue, un statut Proposé / En contrôle / Validé, un commentaire réviseur — voir [GLOSSAIRE.md](GLOSSAIRE.md)) dans un fichier `glossary.json`. L'ancien schéma par langue (v1) est migré au chargement (idempotent, fichier réécrit en v2 à la première sauvegarde) ; les termes migrés naissent Validé pour ne pas changer le comportement des prompts.
- **Deux magasins, un seul à la fois** : le magasin **global** du profil (`%LocalAppData%\CheckTranslation\glossary.json`), et le glossaire **d'une solution** — `<dossier du .sln/.slnx>/.claude/glossary.json`, partagé avec les skills et l'outillage `resx-tools` du dépôt (elec calc). `SolutionGlossaryLocator.Locate(path)` désigne ce dernier dès que la solution possède un répertoire `.claude` (le fichier naît à la première sauvegarde) ; `MainForm` appelle `SwitchStore` après chaque chargement de source, une solution sans `.claude` ramenant au global. Le chemin est injectable par constructeur (`internal GlossaryService(string filePath)`) : c'est ce qui rend le service testable sans toucher au glossaire réel.
- **Sauvegarde déterministe** : termes triés par source, langues par code, statuts en toutes lettres — le fichier d'une solution est versionné avec ses sources, son diff ne doit montrer que ce qui a changé.
- **Cellules à formes multiples** : une traduction peut porter plusieurs formes séparées par « / » (`kabel / kabl`), convention héritée de `glossary.md` d'elec calc. `CanonicalForm` n'injecte dans les prompts que la première ; les suivantes ne servent qu'au contrôle des formes fléchies par `glossary.py check`. L'empreinte et l'export Excel portent la cellule entière.
- `GetTerms()` / `ReplaceTerms(terms)` : la surface de l'éditeur multi-langues, terme entier (copies isolées, normalisation à l'écriture). `AddProposedTerms(candidats)` : versement des candidats d'extraction, multi-langues, en une seule persistance — terme nouveau Proposé, seules les cases vides d'un terme existant se remplissent, contexte vide compris (une valeur tranchée n'est jamais écrasée), compte par terme et non par cellule ; ce qui s'écrit est décidé par `GlossaryCandidates.Classify`, la même classification que les couleurs du dialog de validation. La surcharge par langue `AddProposedTerms(code, entries)` s'y ramène.
...
- L'extraction IA réutilise `Translator.CallApiAsync` (pipeline Polly multi-providers) avec un prompt JSON strict, par lots de `ExtractionBatchSize = 10` textes — via la propriété `ExtractionApiCall`, seule couture qui permette aux tests de rejouer `ExtractCandidatesAsync` de bout en bout sans réseau. **Aucun filtrage des termes déjà connus, ni dans le prompt ni après** : l'IA ne connaît pas le glossaire, ses propositions pour un terme déjà tranché sont confrontées à la valeur du glossaire par le dialog de validation (noir si identique, rouge si différente) — l'extraction est un second regard sur le glossaire. Retourne un `GlossaryExtractionResult` : candidats **et** bilan des lots (échec d'appel, réponse illisible, troncature) — `MainForm` en fait un message d'erreur, d'avertissement (extraction partielle) ou d'information (« l'IA n'a rien trouvé »), jamais le même pour les trois.
...
**`TranslationRowFiltering`** (static) — `Filter(allRows, filters)` sur toutes les colonnes. Un filtre texte est un « contient », sauf s'il commence par `=` : égalité exacte (utilisé par le tableau de bord). Pour `Translation` : `translation:none`, `translation:done`, `translation:same` (identique au français — un signal, pas une erreur), `translation:review` (touchée dans la langue affichée par la dernière passe ciblée — retraduite par la retraduction ciblée, les écarts au glossaire ou la retraduction d'un terme, ou contrôlée depuis un terme du glossaire ; marqueur transitoire par langue sur `TranslationRow` (`MarkForReview` / `IsMarkedForReview` / `ClearReviewMarks`), posé automatiquement en fin de passe pour la relecture). Pour `Comment`, supporte les pseudo-filtres :
...
**`GlossaryImpact`** (static) — cœur pur de la retraduction ciblée : `ComputeChangedTerms(before, after)` compare deux photographies de la projection prompts (par langue) et retient les termes apparus ou modifiés — jamais les supprimés, décision assumée de GLOSSAIRE.md ; `SelectImpactedRows(rows, terms)` sélectionne les lignes dont le français contient un terme, en inclusion insensible à la casse (les formes fléchies éloignées peuvent échapper, limite documentée) ; `RemoveTerm(changedTerms, code, source)` retire d'une détection le terme sur lequel l'utilisateur vient d'agir depuis le menu contextuel de l'éditeur (et la langue devenue vide), pour ne pas lui proposer deux fois les mêmes lignes.

**`GlossaryCandidates`** (static) — extraction multi-langues : `LanguagesWithContent(rows, langues)` retient, dans l'ordre de la toolbar, les langues où au moins une ligne sélectionnée porte une traduction (l'appelant a committé la langue affichée avant) ; `Merge(candidats par langue)` recolle les résultats des appels par langue en termes transversaux, par source normalisée insensible à la casse — premier contexte non vide, destinations vides ignorées, première proposition conservée par langue, ordre de première apparition, statut Proposé. `Classify(candidat, existant)` confronte un candidat au terme de même source du glossaire (`FindExisting`, source normalisée insensible à la casse) et classe chaque cellule et le contexte : `Added` (sera écrit — terme nouveau, ou case vide que la proposition remplit), `Existing` (déjà là, identique après normalisation ou sans proposition), `Conflict` (valeur tranchée différente, casse comprise : la proposition ne l'écrasera pas), `Empty`. `CandidateDiff.AddsAnything` dit si le versement écrirait quelque chose. **Définition unique**, partagée par les couleurs du dialog et par `AddProposedTerms`.

**`GlossaryDeviation`** (static) — écarts au glossaire : lignes traduites dont la traduction n'emploie pas le terme imposé alors que le français contient le terme source. Port à l'identique de `glossary.py check` (resx-tools d'elec calc), pour que l'application et les skills comptent la même chose sur le même `glossary.json` : mots entiers côté français (`FrenchContains`), inclusion insensible à la casse côté cible avec les variantes « / » de la cellule et tolérance sur la dernière lettre d'un mot d'au moins cinq caractères (`TargetContains`), comparaison sur les seuls caractères CJK pour le chinois sauf forme latine, terme le plus long seul contrôlé quand deux se recouvrent (`MatchingTerms`) — **y compris un terme long sans cellule pour la langue**, qui masque le court sans être lui-même contrôlé : d'où `ControlledEntries`, projection de tous les termes Validé cellule vide comprise, là où la projection prompts ne porte que les termes traduits. Une ligne non traduite n'est pas un écart. `SelectDeviationsForTerm(rows, code, entries, source)` restreint le même contrôle à un terme (règle du plus long terme comprise) pour les actions par terme de l'éditeur : pas une règle de plus, la réunion par terme redonne `SelectDeviations`. Fidélité vérifiée sur elec calc : 122 écarts ligne pour ligne des deux côtés sur les dictionnaires `Msg` / `Resources` ; l'application en compte davantage parce qu'elle contrôle aussi les `.resx` de formulaires, que `glossary.py` ignore — différence de périmètre, pas de règle. Toute évolution ici se reporte dans `glossary.py`, et réciproquement.

### 6.5 Modèles

**`TranslationRow`** (POCO) :
- Identité : `Project`, `File`, `Key`.
...
- `SwitchLanguage(oldCol, newCol)` : pousse la vue active dans le dictionnaire sous `oldCol`, recharge depuis `newCol`. Évite toute relecture disque au changement de langue.
...
1. Dialog `OpenFileDialog` (filtre fourni par `TranslationSourceFactory.OpenFileFilter` : `.sln`, `.slnx`) → `_currentFilePath`.
2. `TranslationSourceFactory.Create(path)` construit `_currentSource` (`ResxTranslationSource`).
3. `source.Load(Languages, progress)` charge les lignes (progress bar alimentée par `SourceLoadProgress(Done, Total)` — en fichiers neutres traités).
4. Les entrées `@Invariant` sont ignorées.
5. `row.SelectLanguage(code)` positionne la vue active : la source remplit les dictionnaires par code de langue mais ignore la langue affichée.
6. Les filtres de l'UI sont réinitialisés, `DataSource` assigné, Sauver, les deux boutons du glossaire et le tableau de bord activés (`UpdateSourceDependentButtonsState` : sans source, le glossaire n'a rien à éditer ni à retraduire, le tableau de bord rien à compter).
7. Le glossaire suit la solution : `_glossaryService.SwitchStore(SolutionGlossaryLocator.Locate(path))` bascule sur `.claude/glossary.json` si la solution possède un répertoire `.claude`, sinon sur le magasin global. La status bar l'indique (« glossaire : .claude\glossary.json »). Les empreintes changent avec le magasin : les caches ne resservent rien de l'autre glossaire.
...
- **Repli des projets** : chaque projet se replie et se déplie (bouton de repli, double-clic). Les replis survivent aux reconstructions de l'arbre (filtre du bandeau, F5) ; un filtre du bandeau déplie tout le temps qu'il est posé, ses correspondances doivent se voir, sans oublier les replis (un repli fait sous filtre est transitoire) ; une nouvelle solution repart dépliée. Un rebind de la grille (filtre de colonne, langue, F5) ne déplie rien : seul un clic ou une navigation dans la grille déplie le projet de la ligne courante.
- **Synchronisation de sélection grille ↔ arbre** : sélectionner des lignes dans la grille sélectionne et fait défiler le fichier de la ligne courante dans l'arbre, en dépliant son projet s'il était replié (repli alors oublié) (mono-sélection : l'ancre décide) ; sélectionner un nœud de l'arbre sélectionne dans la grille les lignes affichées du fichier, ou toutes celles du projet, et défile jusqu'à la première. Verrou anti-boucle `_isSyncingSelection` ; un fichier masqué par le bandeau n'est pas synchronisé (le filtre reste maître).
...
- **Extraire les termes métier (langue courante)…** : extraction IA dans la langue affichée, puis `GlossaryExtractionDialog`.
- **Extraire les termes métier (toutes les langues avec du contenu)…** : même extraction, répétée pour chaque langue où la sélection porte une traduction (`GlossaryCandidates.LanguagesWithContent`), candidats fusionnés par terme (`GlossaryCandidates.Merge`) et validés dans un seul dialog à une colonne par langue.
...
Pendant tout l'appel, **toolbar et grille sont gelées** (même mécanisme que l'analyse de mise en page) : les résultats du batch s'écrivent dans la vue active des lignes sélectionnées, et changer de langue ou rafraîchir pendant l'attente corromprait les données (voir §10). F5 est bloqué par `CanRefreshView`, qui vérifie `toolStrip.Enabled`.

### 7.8 Rafraîchir (bouton ou F5)
- **Solution chargée** : `_currentSource.Load(...)` recharge depuis le disque. Détection des lignes dont le français/commentaire source a changé → confirmation utilisateur avant d'écraser les valeurs en mémoire. Les traductions des lignes inchangées sont conservées.
...
- Réécriture des seules variantes de culture réellement modifiées, commentaires de vérification compris ; le fichier neutre n'est jamais touché.
...
### 7.10 Vérification de mise en page
1. `ITranslationSource.SupportsLayoutCheck` vaut `true` pour la source `.resx` ; l'abstraction garde le drapeau pour une source future sans géométrie, où l'analyse serait simplement sautée.
...
### 7.11 Glossaire
- **Édition manuelle** : bouton toolbar `btnGlossary` → `GlossaryForm` (non modal, une instance ; « Ouvrir » et l'extraction de termes sont refusés tant qu'il est ouvert — `SwitchStore` et `AddProposedTerms` écriraient le même fichier que lui), grille transversale terme × langue avec statuts. Seuls les termes Validé sont injectés dans les prompts.
- **Extraction assistée** : menu contextuel sur une sélection, deux portées — la langue affichée, ou toutes les langues où la sélection a du contenu (une traduction non vide sur au moins une ligne). L'extraction reste un appel IA par langue (le prompt cible une langue), sans filtrage des termes déjà connus — leurs propositions servent de contrôle du glossaire dans le dialog ; en mode multi-langues les résultats sont fusionnés en termes transversaux (`GlossaryCandidates.Merge`) et validés dans un seul `GlossaryExtractionDialog` à une colonne par langue → versement en termes **Proposé** (`AddProposedTerms(candidats)`, une persistance), à valider dans l'éditeur ou par le contrôle externe avant d'entrer dans les prompts. L'échec d'une langue n'annule pas les précédentes ; le compte rendu des lots en défaut est préfixé par langue.
...
- **Retraduction des écarts au glossaire** : bouton toolbar `btnGlossaryDeviations` (créé par code, à côté du bouton Glossaire). `BtnGlossaryDeviations_Click` committe la vue active, puis balaye hors du thread d'interface (grille et toolbar gelées, curseur d'attente) chaque langue avec `GlossaryDeviation.SelectDeviations` sur `GlossaryDeviation.ControlledEntries(GetTerms(), code)` — tous les termes Validé, cellule vide comprise, pas la projection prompts —, confirme avec le compte par langue, puis enchaîne sur `RetranslateImpactedAsync`. Rattrape ce que la détection automatique ne voit pas : un glossaire modifié hors de l'éditeur (à la main, par un collègue via git).
- **Retraduction ciblée (phase 4)** : `BtnGlossary_Click` photographie la projection prompts (`GetPromptEntries`, termes Validé par langue) avant et après l'éditeur ; `GlossaryImpact.ComputeChangedTerms` en déduit les termes apparus ou modifiés (les supprimés n'impactent pas), `SelectImpactedRows` sélectionne les lignes par inclusion insensible à la casse du français. Après confirmation (compte par langue), `RetranslateImpactedAsync` retraduit puis re-vérifie langue par langue en écrivant dans les dictionnaires par code, invalide les verdicts de mise en page des traductions changées, recharge la vue active à la fin, puis filtre la grille sur `translation:review` pour la relecture (`ShowRowsToReview`, même mécanique que le drill-down ; reste sur la langue affichée si elle a des lignes marquées, sinon bascule sur la première qui en a — un filtre posé sur une langue sans ligne marquée viderait la grille). La re-vérification se fait comme depuis l'interface, glossaire compris (`BuildGlossarySection(..., forVerification: true)`).
- **Filtre depuis un terme** : « Filtrer la grille principale sur « terme » » dans le menu contextuel de l'éditeur → `MainForm.FilterGridOnFrench` committe l'édition de cellule en cours de la grille principale (l'éditeur est non modal, une saisie peut être ouverte), remet tous les filtres à zéro (`ResetAllFilters` : zones de saisie, listes, cases et filtre du bandeau de l'arborescence — socle partagé avec le drill-down et la relecture) et pose le terme dans la zone de saisie de la colonne Français — filtre « contient » insensible à la casse, même inclusion que `GlossaryImpact.SelectImpactedRows` — puis rend le nombre de lignes affichées, que l'éditeur, resté ouvert, affiche dans son bandeau ; -1 si la grille est gelée (batch, écriture), le bandeau le dit. Un terme commençant par « = » (syntaxe d'égalité exacte du filtre, `TranslationRowFiltering.UsesExactMatchSyntax`) n'est pas filtré et l'éditeur le dit — cas d'école, préféré à un filtre réinterprété en silence. Les gardes qui renvoient vers l'éditeur ouvert (« Ouvrir », extraction, second clic sur le bouton) passent par `BringGlossaryEditorToFront`, qui le restaure s'il est réduit.
- **Actions sur un terme depuis l'éditeur** : le menu contextuel de `GlossaryForm` ferme l'éditeur avec un `GlossaryTermAction` (terme, langue, Contrôler ou Retraduire) ; `BtnGlossary_Click` l'exécute par `RunGlossaryTermActionAsync` **avant** la détection automatique, qui écarte ensuite ce terme dans cette langue (`GlossaryImpact.RemoveTerm`) pour ne pas redemander les mêmes lignes — seulement si la passe a abouti sur au moins une ligne (la méthode rend vrai) : une demande sans objet, refusée, annulée ou sans réponse exploitable laisse la détection proposer normalement. Les lignes visées sont celles dont le français contient le terme (`GlossaryImpact.SelectImpactedRows`, même inclusion que la retraduction ciblée — « disjoncteurs » compte pour « disjoncteur ») ; la confirmation rappelle la cellule imposée et le compte d'écarts rapporté aux seules lignes traduites (`GlossaryDeviation.SelectDeviationsForTerm` sur `ControlledEntries` ; une ligne non traduite n'est ni conforme ni en écart, elle est comptée à part), ou dit pourquoi aucune contrainte ne s'appliquera (cellule vide, terme non Validé donc absent de `GetPromptEntries`). **Contrôler** (`VerifyTermRowsAsync`) bascule la grille sur la langue, vérifie les lignes traduites par `VerifyRowsAsync` (vue active, glossaire avec garde-fou), qui rend les seules lignes dont le score a été recalculé ; celles-là sont poussées dans les dictionnaires, marquées et la grille filtrée sur `translation:review` — une ligne dont la réponse était inexploitable garde son ancien score et n'est pas marquée. **Retraduire** (`RetranslateTermRowsAsync`) propose dans un `TaskDialog` les écarts seuls (par défaut, quand ils sont strictement moins nombreux que les lignes) ou toutes les lignes, puis enchaîne sur `RetranslateImpactedAsync` pour cette seule langue.
...
- …
```
(chaîne vide si aucune entrée → le placeholder disparaît du prompt). Pour la vérification (`forVerification: true`), un paragraphe suit le tableau : le glossaire fixe la terminologie, pas la note — la conformité est une condition nécessaire, jamais suffisante, et ne justifie à elle seule aucun point.
...
- **Modificateurs d'accès** : `internal` pour les classes d'assembly (`TranslationRow`, `AppConfig`, services, modèles…). `public` uniquement pour les classes héritant de `Form`.
- **Records** pour les modèles immuables : `LanguageInfo`, `SourceLoadProgress`, `GlossaryChange`, `GlossaryExtractionResult`.
...
- **Écritures disque protégées par `SetWritingState`** — ne PAS revenir à un `btnXxx.Enabled = false` ponctuel dans `BtnSave_Click` : le verrou global est ce qui bloque aussi la fermeture (`_isWriting` lu par `MainForm_FormClosing`). Toujours appeler `SetWritingState(false)` dans un `finally`.
- **Les batchs IA gèlent toolbar + grille** — `TranslateRowsAsync` / `VerifyRowsAsync` / `ExtractTermsRowsAsync` désactivent `toolStrip` et `dataGridView`, pas seulement Ouvrir/Sauver. Les résultats du batch atterrissent dans la vue active des lignes : changer de langue committerait le placeholder « Traduction en cours... » dans la langue quittée et ferait écrire les résultats dans la nouvelle ; rafraîchir remplacerait `_allRows` et les résultats iraient dans des objets orphelins. Le `finally` ne rouvre pas l'UI si `_isWriting` est posé (même précaution que `RunLayoutCheckAsync`).
...
- **Le chargement et le rafraîchissement gèlent aussi toolbar + grille** — `LoadFileAsync` et `BtnRefresh_Click` désactivent `toolStrip` et `dataGridView` comme les batchs IA : pendant un rechargement, changer de langue mélangerait les vues actives entre anciennes et nouvelles lignes. Les états individuels de Sauver, des boutons du glossaire et du tableau de bord restent gérés séparément (`UpdateSourceDependentButtonsState`) : ils décrivent la source et survivent au dégel du container.
...
- **Une langue est identifiée par son code, nulle part par un numéro** — `TranslationRow.Translations` / `Comments` sont indexés par code (`« de-DE »`), `LanguageInfo` ne porte que `Code` et `Name` (la colonne Excel a disparu avec la source Excel).
...
- **Le glossaire d'une solution est un fichier du dépôt de l'utilisateur** (`.claude/glossary.json`, lu aussi par ses skills) — ne jamais y écrire de fichier annexe : `CreateBackup` écrit dans `%LocalAppData%\CheckTranslation`, pas à côté du magasin, et doit y rester. La sauvegarde reste atomique (`AtomicFile`) et déterministe (tri) : un diff git bruyant ou un fichier tronqué dans le dépôt de l'utilisateur serait pire qu'ici.
- **`SwitchStore` ne sauvegarde rien et n'a rien à sauvegarder** — toutes les écritures du service persistent immédiatement (`ReplaceTermsAndSave`, `AddProposedTerms`, `ExportForReview`) ; ne pas introduire d'état mémoire non persisté, il serait perdu à l'ouverture d'une autre source.
- **Les prompts n'injectent que la forme canonique d'une cellule** — `BuildGlossarySection` passe par `CanonicalForm` (première forme avant « / »). Ne pas appliquer la même coupe à l'empreinte, à l'export Excel ni à `GlossaryImpact` : la cellule entière est la donnée, la coupe n'est qu'un rendu pour le modèle.
...
- **La définition de l'écart au glossaire est partagée avec `glossary.py check`** — `GlossaryDeviation` en est le port fidèle (mots entiers, variantes « / », tolérance de finale à cinq lettres, CJK, plus long terme). Ne pas « améliorer » l'un sans l'autre : l'application et les skills d'elec calc doivent compter les mêmes écarts sur le même fichier, sinon la retraduction des écarts et le contrôle du dépôt se contredisent.
- **La détection automatique de retraduction ne voit que l'éditeur** — `BtnGlossary_Click` photographie avant/après l'éditeur ; un glossaire modifié hors de l'application ne déclenche rien. C'est le bouton « Retraduire les écarts au glossaire » qui couvre ce cas ; ne pas essayer de détecter les changements externes par comparaison au chargement, il n'y a pas de référence fiable.
...
- **La retraduction ciblée écrit dans les dictionnaires par code, jamais dans la vue active** — `RetranslateImpactedAsync` committe d'abord la vue active des lignes impactées de la langue affichée, écrit `Translations[code]` / `Comments[code]` pour chaque langue, invalide le verdict de mise en page des seules traductions réellement changées, et recharge la vue active à la fin. Écrire via `Translation` / `Comment` ne fonctionnerait que pour la langue affichée. Le score d'une ligne retraduite est effacé avant re-vérification : un score de l'ancien glossaire est périmé ; une réponse inexploitable laisse la ligne entière telle quelle (ancienne traduction + ancien score, cohérents entre eux). La re-vérification reçoit le glossaire comme la vérification manuelle, mais **la section de vérification porte un garde-fou** (`BuildGlossarySection(..., forVerification: true)`) : avec le glossaire sous les yeux, le vérificateur constatait la conformité et notait 100 sans juger la langue. Le garde-fou vit dans la section injectée, pas dans le prompt par défaut, pour s'appliquer aussi à un prompt personnalisé déjà enregistré ; ne pas le déplacer dans `DefaultVerifyPrompt`, il n'atteindrait plus les configurations existantes.
- **Les couleurs du dialog d'extraction et le versement partagent `GlossaryCandidates.Classify`** — vert = `Added`, noir = `Existing`, rouge = `Conflict`. Ne pas décider dans le dialog ce qui « sera écrit » avec une autre règle que celle du service, ni l'inverse : un vert qui ne s'écrirait pas, ou un noir qui s'écrirait, ruinerait la confiance dans l'aperçu. Les couleurs restent un aperçu (l'utilisateur peut éditer la source d'une ligne après coloration) : l'autorité est le reclassement à l'écriture, dans `AddProposedTerms`. **L'extraction ne doit pas refiltrer les termes déjà connus** (ni dans le prompt, ni après la réponse) : cas vécu, le rouge était inatteignable et un terme connu dans toutes les langues n'apparaissait plus — le second regard sur le glossaire passe précisément par les propositions faites sur des termes déjà tranchés.
- **L'éditeur de glossaire est non modal : ce qui suivait `ShowDialog` vit dans `GlossaryForm_FormClosed`** — action demandée sur un terme, puis détection des lignes impactées à partir de la photographie prise à l'ouverture (`_glossaryProjectionBefore`). La grille peut être gelée au moment de la fermeture (batch lancé depuis la grille pendant que l'éditeur était ouvert) : le gestionnaire teste `toolStrip.Enabled` et `_isWriting` et met alors la suite **en attente** (`_pendingGlossaryFollowUp` : photographie + action), lancée au dégel par `toolStrip.EnabledChanged` → `BeginInvoke(RunPendingGlossaryFollowUp)` — après le `finally` qui a rouvert l'UI, et en revérifiant le gel. Ne pas jeter la photographie dans ce cas : un changement de contexte seul, ou un terme dont les traductions sont déjà conformes, échapperaient au bouton « Retraduire les écarts au glossaire », qui ne voit que les destinations manquantes. Ne pas revenir à `ShowDialog` sans retirer le filtre par terme, qui n'a de sens que fenêtre ouverte ; ne pas ouvrir une seconde instance (`_glossaryForm`).
- **« Ouvrir » et l'extraction de termes sont refusés tant que l'éditeur de glossaire est ouvert** — `SwitchStore` rebrancherait le service sur un autre fichier sous un éditeur qui y enregistrerait l'ancien contenu ; `AddProposedTerms` verserait des termes que l'éditeur, périmé, écraserait à son enregistrement. `BtnOpen_Click` et `ExtractTermsFromContextSelectionAsync` testent `_glossaryForm` et ramènent l'éditeur au premier plan. Rafraîchir (F5) reste permis : même fichier, l'éditeur n'est pas concerné.
- **Un bouton à `DialogResult` ne ferme pas une fenêtre non modale** — seul `ShowDialog` s'arrête sur `DialogResult`. Dans `GlossaryForm`, `btnCancel` a un `Click` explicite et `BtnOk_Click` / `RequestTermAction` appellent `Close()` ; ne pas retirer ces appels en croyant le `DialogResult` suffisant.
- **Le marqueur « à relire » est unique et transitoire** — `TranslationRow.MarkForReview` / `translation:review` servent à toutes les passes ciblées (retraduction ciblée, écarts au glossaire, actions par terme de l'éditeur : retraduction comme contrôle). Chaque passe commence par `ClearReviewMarks` sur toutes les lignes : la relecture ne montre que ce que la dernière passe a fait. Ne pas créer un second marqueur par type de passe, ni le persister — c'est un état de session, comme les caches. Un contrôle depuis un terme écrit dans la vue active (`VerifyRowsAsync`) : le commit dans le dictionnaire de la langue puis `SelectLanguage` qui suivent sont ce qui fait apparaître le marqueur, ne pas les retirer ; et seules les lignes que `VerifyRowsAsync` rend (score recalculé) sont marquées — marquer la liste demandée ferait relire des lignes dont rien n'a changé.
- **Une action exécutée depuis l'éditeur écarte sa paire (terme, langue) de la détection automatique** — `ProposeTargetedRetranslationAsync(projectionBefore, alreadyRequested)` passe par `GlossaryImpact.RemoveTerm`. Sans cela, corriger la cellule allemande de « disjoncteur » puis cliquer « Retraduire … en Allemand » produirait deux confirmations pour les mêmes lignes. Seule une passe qui a abouti sur au moins une ligne compte (`RunGlossaryTermActionAsync` rend vrai ; `VerifyRowsAsync` rend les lignes vérifiées, `RetranslateImpactedAsync` rend vrai si une ligne a été retraduite) : une demande refusée, annulée, sans configuration API, sans ligne concernée ou sans réponse exploitable passe null, et le changement du glossaire reste proposé — sinon un clic sur Annuler ferait disparaître une retraduction due. Les autres changements faits dans l'éditeur restent proposés normalement.
...
- **Un filtre texte préfixé de `=` est une égalité exacte** — la saisie manuelle reste un « contient ». Depuis que Projet et Fichier n'ont plus de colonne, le drill-down du tableau de bord ne pose plus ces filtres en texte : il passe par l'arborescence (`SelectTreeExactly`), qui porte la même exigence — un fichier n'est identifié que par son projet *et* son chemin, et `Properties\Msg` ne doit pas ramener `Properties\Msg2`. Le support du `=` reste dans `TranslationRowFiltering` pour les autres colonnes ; la règle est exposée par `UsesExactMatchSyntax` pour qui pose un texte littéral dans une zone de filtre (filtre par terme du glossaire) et doit savoir qu'il serait réinterprété.
- **`ApplyDrillDown` remet tous les filtres à zéro avant de poser le sien** — un filtre resté en place afficherait moins de lignes que le chiffre cliqué, et le tableau de bord passerait pour faux. L'arborescence de la solution en fait partie : cases recochées et filtre du bandeau vidé. Le socle est `ResetAllFilters`, partagé avec la relecture (`ShowRowsToReview`) et le filtre par terme (`FilterGridOnFrench`) : ne pas réécrire la remise à zéro site par site.
- **L'arborescence de la solution est un filtre d'affichage, pas une exclusion** — décocher un fichier le retire de la grille, mais la sauvegarde, le tableau de bord et l'analyse de mise en page portent toujours sur toutes les lignes (`_allRows`). Ne pas brancher ces opérations sur `GetTreeVisibleRows` sans repenser les invariants du tableau de bord.
...
- **Les replis de l'arbre vivent dans `_collapsedProjects`, jamais dans les nœuds** — même raison que les cases : la reconstruction recrée les nœuds. `RebuildSolutionTreeNodes` replie les projets mémorisés seulement sans filtre du bandeau (un filtre déplie tout, ses correspondances doivent se voir, les replis restent mémorisés) ; `AfterCollapse` / `AfterExpand` ne mettent le jeu à jour que hors reconstruction (`_isUpdatingTreeChecks`) et **hors filtre du bandeau** (`RememberProjectCollapse` : sous filtre, un repli est transitoire et l'état d'avant le filtre reste intact). La sélection grille → arbre et le drill-down déplient le projet visé et oublient son repli — mais **jamais sur un rebind** : `ApplyFilters` et la restauration de sélection d'`ApplyFiltersPreservingSelection` posent `_isRebindingGrid`, et `SyncTreeSelectionFromGrid` laisse alors un projet replié tranquille, sans sélectionner le nœud caché (ce qui le dévoilerait) ; sans ce drapeau, taper dans un filtre de colonne, changer de langue ou F5 redéplierait le projet de la ligne courante. `SolutionTree_AfterSelect` ignore les sélections induites par un repli ou un dépliage (`TreeViewAction.Collapse` / `Expand`) : WinForms remonte la sélection sur le projet quand on replie celui du fichier sélectionné, et le gestionnaire sélectionnerait toutes ses lignes dans la grille sans que l'utilisateur ait rien choisi. Les boutons de repli des racines exigent `ShowLines` **et** `ShowRootLines` (Win32 : `TVS_LINESATROOT` n'agit qu'avec `TVS_HASLINES`) ; le thème Explorer posé dans `CheckBoxTreeView.OnHandleCreated` remplace les pointillés par des flèches — ne pas remettre `ShowLines = false`, les projets redeviendraient non repliables à la souris.
...
**Couverture actuelle** (logique pure uniquement, aucun test d'UI) : `QualityScore` (parsing + palette), `TranslationRowFiltering` (contient / `=` exact / pseudo-filtres score, traduction, layout), `TranslationStatistics` (comptages dans les lignes traduites, moyennes nulles, cohérence tranche comptée / tranche filtrée), `GlossaryDiff` (types de changements, statuts, promotion sous couverture complète, doublon refusé), `GlossaryImpact` (projection avant/après, suppressions ignorées, sélection insensible à la casse, retrait d'un terme déjà traité par `RemoveTerm`), `GlossaryExcel` (aller-retour sur fichiers temporaires, refus des classeurs ambigus), `Translator.ParseNumberedList` (comportements caractérisés, dont : un numéro hors bornes est rattaché à l'entrée courante), caches de `TranslationService` (clés par langue + fingerprint, purge par modèle), `GlossaryService.NormalizeCell` et `GlossaryService.ParseExtractionResponse` (JSON dans de la prose ou des fences, tableau vide = succès, troncature détectée avec ou sans `]` intérieur survivant), magasin du glossaire sur chemin injecté (`GlossaryStoreTests` : tri et statuts en texte à la sauvegarde, création au premier enregistrement, lecture d'un JSON migré à la main, forme canonique dans les prompts, `SwitchStore` et empreintes, `SolutionGlossaryLocator`), `GlossaryDeviation` (comportements portés de `glossary.py` : mots entiers, variantes, tolérance de finale, CJK et sigles latins, plus long terme, lignes non traduites ignorées ; `SelectDeviationsForTerm` restreint à un terme avec masquage par le plus long, réunion égale à la liste globale, rien sans cellule), `GlossaryCandidates` (langues avec contenu, fusion des candidats par source insensible à la casse, premier contexte, première proposition par langue ; `Classify` : terme nouveau, case vide remplie, identique, conflit y compris de casse, contexte vide rempli, rien à ajouter) et le versement multi-langues `AddProposedTerms(candidats)` (terme créé Proposé, cases et contexte vides seuls complétés, valeur tranchée jamais écrasée, statut conservé, compte par terme, rien d'écrit quand il n'y a rien à remplir), et `ExtractCandidatesAsync` rejoué sans réseau via `ExtractionApiCall` (un terme déjà au glossaire reste candidat et le glossaire n'est pas soufflé au prompt ; déduplication et compte des lots à travers deux lots).

**Prochaines extensions naturelles** : `LayoutAnalyzer` (la mesure injectée `TextWidthMeasurer` existe pour ça), `AppConfig` (round-trip DPAPI + compat legacy — attention : ne jamais écrire dans le vrai `%LocalAppData%`). `GlossaryService` se teste avec son constructeur à chemin explicite (fichier temporaire) ; ne jamais y appeler `CreateBackup` ni `SwitchStore(null)` suivi d'une lecture : les deux toucheraient le profil réel de l'utilisateur.
