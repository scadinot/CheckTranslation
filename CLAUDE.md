...
- **Filtre depuis un terme** : « Filtrer la grille principale sur « terme » » dans le menu contextuel de l'éditeur → `MainForm.FilterGridOnFrench` remet tous les filtres à zéro (arborescence comprise, même mécanique que le drill-down) et pose le terme dans la zone de saisie de la colonne Français — filtre « contient » insensible à la casse, même inclusion que `GlossaryImpact.SelectImpactedRows` — puis rend le nombre de lignes affichées, que l'éditeur, resté ouvert, affiche dans son bandeau ; -1 si la grille est gelée (batch, écriture), le bandeau le dit.
