...
- **Les dialogs se bornent à l'écran via `ScreenFit.Apply(this)` dans `OnLoad`** — helper partagé (`Forms/ScreenFit.cs`, ex-`FitToWorkingArea` de `ConfigForm`) : `MinimumSize` bornée avant la fenêtre, écran de référence = celui du `Owner`. Tout nouveau dialog plus grand que ~1000 px doit l'appeler.
