...
- L'extraction IA réutilise `Translator.CallApiAsync` (pipeline Polly multi-providers) avec un prompt JSON strict, par lots de `ExtractionBatchSize = 10` textes. **Aucun filtrage des termes déjà connus, ni dans le prompt ni après** : l'IA ne connaît pas le glossaire, ses propositions pour un terme déjà tranché sont confrontées à la valeur du glossaire par le dialog de validation (noir si identique, rouge si différente) — l'extraction est un second regard sur le glossaire. Retourne un `GlossaryExtractionResult` : candidats **et** bilan des lots (échec d'appel, réponse illisible, troncature) — `MainForm` en fait un message d'erreur, d'avertissement (extraction partielle) ou d'information (« l'IA n'a rien trouvé »), jamais le même pour les trois.
