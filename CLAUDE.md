...
- **Retraduction des écarts au glossaire** : bouton toolbar `btnGlossaryDeviations` (créé par code, à côté du bouton Glossaire). `BtnGlossaryDeviations_Click` committe la vue active, sélectionne pour chaque langue les lignes en écart (`GlossaryDeviation.SelectDeviations` sur `GetPromptEntries`), confirme avec le compte par langue, puis enchaîne sur `RetranslateImpactedAsync`. Rattrape ce que la détection automatique ne voit pas : un glossaire modifié hors de l'éditeur (à la main, par un collègue via git).
