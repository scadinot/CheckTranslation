...
| 2026-09-14 | Arborescence : projets repliables — boutons de repli des nœuds racine (`ShowLines` + `ShowRootLines`, règle Win32 `TVS_LINESATROOT`) avec thème Explorer (`SetWindowTheme`) pour des flèches sans pointillés ; replis mémorisés dans `_collapsedProjects` hors des nœuds et conservés à travers les reconstructions (filtre du bandeau — qui déplie tout le temps qu'il est posé —, F5), oubliés à l'ouverture d'une autre solution ; la sélection d'une ligne dans la grille déplie le projet de son fichier (repli oublié), le drill-down aussi ; une sélection induite par un repli (`TreeViewAction.Collapse`) ne sélectionne plus toutes les lignes du projet dans la grille. Suite revue Copilot : un rebind de la grille (`_isRebindingGrid`, posé par `ApplyFilters` et la restauration de sélection) ne déplie pas un projet replié, et un repli fait sous filtre du bandeau est transitoire (`RememberProjectCollapse`) |
| 2026-09-14 | Éditeur de glossaire non modal (`Show(owner)`, une instance, suite de la fermeture dans `GlossaryForm_FormClosed`) et menu contextuel « Filtrer la grille principale sur « terme » » : la grille ne montre plus que les lignes dont le français contient le terme, sans fermer l'éditeur, qui affiche le compte dans son bandeau (`MainForm.FilterGridOnFrench`, tous filtres remis à zéro, terme posé dans la zone de filtre Français). Garde-fous : « Ouvrir » et l'extraction de termes refusés tant que l'éditeur est ouvert (`SwitchStore` / `AddProposedTerms` écriraient le même fichier), rien n'est lancé à la fermeture si la grille est gelée, `btnCancel` ferme explicitement (un `DialogResult` ne ferme pas une fenêtre non modale). Suite revue Copilot : langue de repli du menu lue au clic (`MainGridLanguageCode`, la grille peut changer de langue éditeur ouvert), édition de cellule en cours committée avant le filtre, remise à zéro des filtres factorisée dans `ResetAllFilters` (bandeau de l'arborescence compris, partagé avec le drill-down et la relecture), éditeur réduit restauré avant `Activate` (`BringGlossaryEditorToFront`), terme commençant par « = » signalé non filtrable (`TranslationRowFiltering.UsesExactMatchSyntax`, testé), suite d'un éditeur fermé pendant un gel mise en attente et lancée au dégel (`_pendingGlossaryFollowUp`, `toolStrip.EnabledChanged`) au lieu d'être abandonnée |
| 2026-09-14 | Suite revue Copilot de la PR #60 : l'exclusion de la paire terme / langue de la détection automatique ne s'applique qu'à une passe qui a abouti sur au moins une ligne (`RunGlossaryTermActionAsync` rend vrai, `VerifyRowsAsync` rend les lignes vérifiées, `RetranslateImpactedAsync` rend vrai si une ligne a été retraduite) — un clic sur Annuler ne masque plus une retraduction due ; écarts rapportés aux seules lignes traduites, non traduites comptées à part ; seules les lignes réellement vérifiées marquées à relire ; `GlossaryForm` committe l'édition en cours avant de lire la source du menu et tient la ligne par référence (le commit peut re-trier la grille), champs de contrôles sans underscore |
| 2026-09-14 | Éditeur de glossaire : menu contextuel d'un terme — « Contrôler les traductions de « terme » en <langue> » (vérification IA des traductions existantes des lignes dont le français contient le terme) et « Retraduire … » (retraduction + re-vérification, écarts au glossaire seuls ou toutes les lignes, choix dans un `TaskDialog`), langue de la colonne cliquée ou langue affichée. L'éditeur enregistre d'abord (confirmation) puis se ferme en portant la demande (`GlossaryTermAction`), `MainForm` l'exécute avant la détection automatique, qui écarte cette paire terme / langue (`GlossaryImpact.RemoveTerm`). `GlossaryDeviation.SelectDeviationsForTerm` restreint le contrôle à un terme (testé). Le marqueur de relecture devient générique : `translation:review` remplace `translation:retranslated`, posé aussi après un contrôle |
| 2026-09-14 | Extraction sans filtrage des termes connus : le prompt ne recevait plus la liste des termes du glossaire (bloc « ne pas ré-extraire ») et le service écartait les propositions sur des termes déjà traduits — le rouge du dialog était inatteignable et un terme connu dans toutes les langues n'apparaissait plus. Désormais l'IA propose tout, le dialog confronte (noir identique, rouge différent) : l'extraction devient un second regard sur le glossaire. `AlreadyKnown` retiré du bilan d'extraction |
| 2026-09-14 | Suite revue Copilot de la PR #57 : l'en-tête du dialog d'extraction adapte la hauteur de son panneau au texte replié (`FitHeaderHeight`, à l'affectation et au redimensionnement) au lieu d'une hauteur fixe qui rognait à largeur minimale ; contrat de `IGlossaryService.AddProposedTerms` réaligné (contexte vide d'un terme existant rempli, règle de non-écriture précisée) |
| 2026-09-14 | Dialog d'extraction : cellules colorées d'après le glossaire courant — vert sera ajouté, noir déjà au glossaire (affiché tel quel, lecture seule), rouge diffère d'une valeur déjà tranchée et ne sera pas appliqué (valeur en infobulle) ; ligne sans rien à ajouter décochée, en-tête avec comptes et légende. La classification (`GlossaryCandidates.Classify`, testée) est partagée avec `AddProposedTerms`, qui remplit désormais aussi le contexte vide d'un terme existant — une seule définition de ce qui s'écrit |
| 2026-09-14 | Extraction des termes métier en deux portées : la langue affichée (comme avant, l'entrée de menu la nomme) ou toutes les langues où la sélection a du contenu (nouvelle entrée du menu contextuel). Un appel IA par langue, candidats fusionnés par terme (`GlossaryCandidates`, logique pure testée), validation dans un seul `GlossaryExtractionDialog` à colonnes de langue dynamiques (grille non liée), versement multi-langues en une persistance (`AddProposedTerms(candidats)`, cases vides seules complétées). L'échec d'une langue conserve les précédentes |
| 2026-09-08 | Retraduction ciblée : à la fin de la passe la grille est filtrée sur les lignes retraduites (`translation:retranslated` — renommé `translation:review` le 2026-09-14, voir plus haut ; marqueur transitoire par langue sur `TranslationRow`, bascule sur la première langue qui en a) pour les relire. Vérification : la section glossaire injectée en vérification (`BuildGlossarySection(..., forVerification: true)`, manuelle comme après retraduction) porte un garde-fou — la conformité au glossaire ne justifie à elle seule aucune note, le vérificateur qui l'avait sous les yeux notait 100 sans juger la langue |
| 2026-09-08 | Bouton État des traductions grisé sans solution chargée, comme Glossaire et Écarts au glossaire : `UpdateGlossaryButtonsState` devient `UpdateSourceDependentButtonsState` et couvre les trois boutons |
| 2026-09-08 | Icône d'ouverture : l'icône Solution de Visual Studio fournie par l'utilisateur remplace le dessin de repli — fond rendu transparent par remplissage depuis les bords (l'intérieur du document reste opaque), recadrée au carré (33 px), `LoadIcon` la ramène à 24 px |
